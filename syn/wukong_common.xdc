...
#
# The handful of pins that differ between revisions live in wukong_v1.xdc and
# wukong_v3.xdc; syn/build.tcl reads the revision file first and then this one.
# That order matters and is not cosmetic -- see the note on the MII clocks
# below.  Everything here is common because the two boards genuinely agree:
...
# LEDs and button
# ---------------------------------------------------------------------------
# The on-board LEDs and the two buttons are in the revision file: they are the
# only things besides the oscillator that QMTech moved between V1 and V3.
#
# The Sun-2 front panel diagnostic LEDs, on PMOD J10.  Identical on V1 and V3 --
# the two hardware manuals give the same J10 table line for line.
...
# ---------------------------------------------------------------------------
# Lives in its own file per bridge -- wukong_wbcdc.xdc for wb_to_mig_ui's
# toggle handshake, wukong_wbfifo.xdc for sun2_fifo_bridge's FIFOs -- because
# each names cells the other build does not have, and an XDC cannot say `if'.
