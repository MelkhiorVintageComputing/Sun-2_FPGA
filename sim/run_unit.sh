...
orphanfifo)
	...
	;;
orphancached)
	# The same, through the cached bridge.
	# The same scenario through the FIFO bridge: its Wishbone side on ui_clk,
	# wb_mig_sync with no crossing, and a MIG model that stalls at random.
	[ -f "$top/build/inputs/z8530_scc/z8530_scc.sv" ] || "$top/tools/patch_inputs.sh" z8530_scc
	make -s -C "$top/tools"
	odefs=(-d SUN2_SIM -d SUN2_MULTIBUS -d MEM_PAGES=512 -d SRAM_POWERUP_ZERO -d SUN2_WB_FIFO -d SUN2_WB_CACHE)
	step xvlog "${odefs[@]}" -i "$top/rtl/sun2-common" -i "$top/build/rom" \
		"$top/rtl/sun2-common/sun2_fpga.v" "$top/rtl/sun2-common/sun2_mmu.v" \
		"$top/rtl/sun2-common/ctx_reg.v" "$top/rtl/sun2-common/pmap.v" \
		"$top/rtl/sun2-common/smap.v" "$top/rtl/sun2-common/sram_sync.v" \
		...
		"$top/rtl/sun2-common/ttl_am9513.v" "$top/rtl/sun2-common/mm58167.v" \
		"$top/rtl/sun2-common/ttl_74F151.v" "$top/rtl/sun2-common/ttl_74LS148.v" \
		"$top/rtl/sun2-common/sun2_async_fifo.v" "$top/rtl/sun2-common/sun2_fifo_bridge.v" "$top/rtl/sun2-common/sun2_cached_fifo_bridge.v" \
		"$top/rtl/sun2-common/tolog.v"
	step xvlog --sv "${odefs[@]}" -i "$top/rtl/sun2-common" -i "$top/build/rom" \
		"$top/build/inputs/z8530_scc/z8530_scc.sv" \
		"$top/boards/Wukong/wb_mig_sync.sv" "$top/boards/Wukong/mig_arb.sv" \
		"$top/tb/mig_ui_model.sv" "$top/tb/tb_orphan_ack.sv"
	step xelab -debug off --timescale 1ns/1ps work.tb_orphan_ack -s orphancached_sim
	xsim orphancached_sim -R | grep -E '===|PASS|FAIL|gap|  [ 0-9]+ |adapter|words of|setup|no response|FATAL|Error'
	;;
...
migddr3fifo)
	...
	xsim migddr3fifo_sim -R ${XSIMARGS:-} | grep -vE 'ddr3\.(cmd_task|data_task|reset|dqs_)' \
		| grep -E '===|calibration|written and read|MISMATCH|PASS|FAIL|latency|^MIG read|^FIFO bridge|^Cached bridge|^=>|scan-out'
	;;
migddr3cached)
	# The same, through the cached bridge.
	# The same, through sun2_fifo_bridge and wb_mig_sync: what the machine waits.
	...
	mapfile -t mig_src < <(find "$mig" -name '*.v' -o -name '*.sv' \
	                       | grep -v '/sun2_mig_mig\.v$' | sort)
	step xvlog -i "$top/rtl/sun2-common" "$top/rtl/sun2-common/sun2_async_fifo.v" "$top/rtl/sun2-common/sun2_cached_fifo_bridge.v"
	step xvlog --sv -d SUN2_WB_FIFO -d SUN2_WB_CACHE -i "$ex" -i "$mig" \
		"$top/boards/Wukong/wukong_clkgen.sv" \
		"$top/boards/Wukong/wb_mig_sync.sv" \
		"$top/boards/Wukong/mig_arb.sv" \
		"${mig_src[@]}" \
	...
	step xvlog "$XILINX_VIVADO/data/verilog/src/glbl.v"
	step xelab -debug off -L unisims_ver -L unisim -L secureip \
		work.tb_mig_ddr3 work.glbl -s migddr3cached_sim
	xsim migddr3cached_sim -R ${XSIMARGS:-} | grep -vE 'ddr3\.(cmd_task|data_task|reset|dqs_)' \
		| grep -E '===|calibration|written and read|MISMATCH|PASS|FAIL|latency|^MIG read|^FIFO bridge|^Cached bridge|^=>|scan-out'
	;;
*)
